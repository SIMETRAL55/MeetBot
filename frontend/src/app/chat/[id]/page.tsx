...
import { Send, User, Bot, Loader2, ArrowLeft, Settings2, FileText, ChevronDown, ChevronRight, SlidersHorizontal, Network } from "lucide-react";
...
function SourceAccordion({
  ...
  const label = isPageIndex
    ...
    : `Retrieved Context (${sources.length} item${sources.length !== 1 ? "s" : ""})`;
    ...
    <div className="mt-4 rounded-md border border-slate-700 bg-slate-800/50">
      ...
      >
        <span className="flex items-center gap-2">
          <FileText className={`h-3.5 w-3.5 ${isPageIndex ? "text-amber-400" : "text-teal-400"}`} />
          {label}
          {isPageIndex && (
            <span className="rounded bg-amber-500/10 px-1 py-0.5 text-[10px] font-semibold text-amber-400 ring-1 ring-amber-500/20">
              PageIndex
            </span>
          )}
        </span>
        ...
        <div className="border-t border-slate-700 px-3 py-2">
          {sources.map((src, i) => (
            <div key={i} className="mb-3 last:mb-0">
              <div className="mb-1 flex items-center gap-2 text-[10px] font-semibold tracking-wider text-slate-400">
                {isPageIndex && src.node_title ? (
                  <>
                    ...
                    {src.node_id && (
                      <span className="rounded bg-slate-900 px-1 py-0.5 text-slate-500">
                        node {src.node_id}
                      </span>
                    )}
                  </>
                ) : (
                  <span className="rounded bg-slate-900 px-1 py-0.5" title="Distance (lower is better)">
              ...
              <p className={`border-l-2 pl-2 text-xs leading-relaxed text-slate-300 ${isPageIndex ? "border-amber-500/50" : "border-teal-500/50"}`}>
                &quot;{src.text}&quot;
              </p>
            </div>
          ))}
...
export default function ChatPage() {
  ...
  useEffect(() => {
    ...
    api.getJobStatus(id).then((job: Job) => {
      ...
      setPageindexAvailable(job.pageindex_status === "ready");
    }).catch((err: any) => {
  ...
  // If PageIndex becomes unavailable, fall back to vector
  useEffect(() => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    sendMessage(
      input,
      llmMode,
      retrievalMethod === "pageindex" ? "chunk" : retrievalLevel,
      retrievalLevel === "segment" ? segmentCount : undefined,
      retrievalMethod,
    );
    setInput("");
  };
      ...
      <div className="relative z-10 mb-6 flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        ...
        <div className="flex flex-col gap-3">
          ...
          <div className="flex items-center gap-3 rounded-lg border border-slate-700 bg-slate-900/50 p-2 text-sm shadow-sm backdrop-blur">
            ...
            <div className="flex rounded bg-slate-950 p-1">
              <button
                onClick={() => setRetrievalMethod("vector")}
                className={`flex-1 rounded-sm px-3 py-1 text-xs font-semibold transition-colors ${retrievalMethod === "vector" ? "bg-slate-800 text-teal-400 shadow" : "text-slate-500 hover:text-slate-300"}`}
              >
                Vector
              </button>
              <button
                onClick={() => { if (pageindexAvailable) setRetrievalMethod("pageindex"); }}
                disabled={!pageindexAvailable}
                title={!pageindexAvailable ? "Build PageIndex on the job detail page first" : "Use PageIndex tree search"}
            ...
            {!pageindexAvailable && (
              <span className="text-[10px] text-slate-600 italic">not built</span>
            )}
          </div>

          {/* Retrieval level selector — hidden when PageIndex is active */}
          {retrievalMethod === "vector" && (
